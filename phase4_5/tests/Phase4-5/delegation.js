...
function println() {
    ...
    for (local i = 0; i < argc; ++i)
        ::print(argument(i));::print(::nl);
}

function nothing() {}
dprintln = nothing;
if (DEBUG) {::dprintln = ::println;
}
...
function objecthaskey(obj, key) {
    ...
    objectforeach(obj, (function(closure, key, val) {::dprintln("closure.has_key(", closure.has_key, ") = key(",
                key, ") == closure.key(", closure.key, ") -> ",
                (key == closure.key));
            return not(closure.has_key = key == closure.key);
        }),
        closure);::dprintln("closure = ", closure);
...
function objectfind(obj, predicate, closure) {
    local fclosure = [
        { "closure": closure }, { "predicate": predicate }
    ];
...
function createTable() {
    return [
        { "append": table_append }, { "foreach": table_foreach }, { "find": table_find }, { "findif": table_findif }
        // state
        , { "__nextIndex": 0 }, { "__data": [] }
    ];
}
...
methodargs = nil;
////////////////////////////
granchoild..delegate(fother);::print("---------------------------\nShould print \"fother loo called.\": ");
play_with_granchoild();


////////////////////////////
granchoild..delegate(mather);::print("---------------------------\nShould print \"fother loo called.\": ");
play_with_granchoild();